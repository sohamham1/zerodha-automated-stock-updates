...
def build_confidence(evidence, consensus):
    ...
    return "low", "Confidence is low because evidence is sparse, stale, or supported by very limited broker coverage."
...
def build_summary(holding, report_date):
    ...
    if int(consensus.get("scannedCount") or 0) >= 2:
        ...
        reasons.append("external broker coverage is still thin")
    ...
    if int(consensus.get("scannedCount") or 0) < 2:
        missing_evidence.append("Broker coverage is limited, so consensus signals are incomplete.")
    ...
    if int(consensus.get("scannedCount") or 0) < 2:
        watchpoints.append("Independent broker confirmation is limited, so treat outside consensus as directional only.")
    if not watchpoints:
